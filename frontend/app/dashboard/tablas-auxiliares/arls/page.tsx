...
import { Shield, ArrowLeft, Power, PowerOff, Download } from 'lucide-react'
...
export default function ARLsPage() {
  ...
  const handleDownload = async () => {
    try {
      ...
      showToast('Archivo Excel de ARLs descargado exitosamente', 'success')
    } catch (error) {
  ...
  const handleActivateDeactivate = async () => {
    ...
    try {
      ...
      localStorage.removeItem('arls_cache')
      await loadData()
      showToast(`ARL ${newStatus ? 'activada' : 'desactivada'} exitosamente`, 'success')
      setShowActivateDeactivateModal(false)
      setSelectedRecord(null)
    } catch (error: any) {
      console.error('Error updating arl:', error)
      showToast(error.message || 'Error al actualizar la ARL', 'error')
    } finally {
      setUpdating(false)
    }
  }
      ...
      <div className="mb-6">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <button
            ...
            title="Descargar lista de ARLs en formato Excel"
          >
