...
/**
 * Página principal del módulo Contratación
 * Enfocada en crear nuevos contratos y seguir el proceso de onboarding
 * Solo muestra contratos con onboarding incompleto (< 100%)
 */
export default function ContratacionPage() {
  ...
  const loadContracts = async () => {
    ...
    const cachedData = localStorage.getItem('contracts_contratacion_cache')
    ...
    try {
      ...
      // Obtener los contratos (sin computed columns para evitar errores de cache)
      ...
      if (contractsError) {
        ...
        if (contractsError.code === 'PGRST116' || contractsError.message?.includes('relation "contracts" does not exist')) {
          console.warn('⚠️ La tabla contracts no existe aún.')
          setContracts([])
          setDataLoaded(true)
          setLoading(false)
          setLoadingRef(false)
      ...
      // FILTRAR: Solo contratos con onboarding incompleto (< 100%)
      const contractsInOnboarding = enrichedContractsData.filter(contract => {
        const progress = contract.contracts_onboarding_progress || 0
        return progress < 100
      })
      
      // Ordenar alfabéticamente por nombre de empresa cliente
      const sortedContractsData = contractsInOnboarding.sort((a, b) => {
      ...
      // Guardar en cache
      localStorage.setItem('contracts_contratacion_cache', JSON.stringify({
  ...
  const handleModalSuccess = () => {
    ...
    localStorage.removeItem('contracts_contratacion_cache')
    localStorage.removeItem('contracts_cache_v3') // También limpiar cache de contratos
    setDataLoaded(false)
    setLoading(true)
      ...
      <div className="space-y-6">
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
          ...
          <p className="text-red-600">
            No tienes permisos para acceder al módulo de contratación.
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center space-x-3">
            <UserPlus className="h-8 w-8 text-[#87E0E0]" />
      ...
      {loading ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          <div className="w-8 h-8 border-4 border-[#87E0E0] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-gray-600">Cargando contratos en proceso de contratación...</p>
        </div>
      ) : contracts.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          <div className="w-24 h-24 bg-gradient-to-br from-[#87E0E0] to-[#5FD3D2] rounded-full flex items-center justify-center mx-auto mb-6">
            <UserPlus className="h-12 w-12 text-[#004C4C]" />
          </div>
          ...
          <p className="text-gray-600 mb-6 max-w-md mx-auto">
            {dataLoaded ? 
              'No hay contratos en proceso de contratación. Crea un nuevo contrato para comenzar el proceso de onboarding.' :
              'Configurando el módulo de contratación...'
            }
          </p>
